...
def make_fgmt(sess, env, X_data, epochs=1, eps=0.01, batch_size=128):
    ...
    for batch in range(n_batch):
        ...
        adv, adv1, adv2 = sess.run(env.x_fgmt, feed_dict={env.x: X_data[start:end], env.fgsm_eps: eps, env.fgsm_epochs: epochs})
        # 衡量重加噪有效性的
        # 第一个模型
        X_adv[start:end] = adv
        # 衡量多数表决有效性
...
def evaluate(sess, env, X_data, y_data, batch_size=128):
    ...
    print(' loss: {0:.4f} acc: {1:.4f} loss1: {2:.4f} acc1: {3:.4f} loss2: {4:.4f} acc2: {5:.4f}'.format(loss, acc, loss1, acc1, loss2, acc2))
    ...
    for my_i in range(100, 200):
        ...
        for my_j in range(1, 2):
            for my_k in range(0, 13):
                ...
                print(
                    'rateA = {0}, stride = {1}, distance = {2}, 扰动率 = {3}, 扰动强度= {4}, necessity = {5}'.format(my_j, my_k, a2, my_j / 784,
                                                                                             my_k / 12, a1))
...
#todo 训练与对抗训练计时操作
    ...
    X_adv = make_fgmt(sess, my_env_1, X_test, eps=0.22, epochs=1)
...
# #
    ...
    z_voting_use_softmax_if_three_not_same_use_probability = voting_algorithm_if_three_not_same_use_probability(y2,y22,y222,z2,z22,z222,10000)
